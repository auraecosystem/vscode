...
suite('CopilotGitHubTokenProvider', () => {
	...
	test('cancels a refresh that does not complete in time', async () => {
		const tokenProvider = disposables.add(new CopilotGitHubTokenProvider(() => 1_000_000, 0));
		tokenProvider.updateToken('expiring-token', 3600);
		let refreshRequests = 0;
		disposables.add(tokenProvider.onDidRequestRefresh(() => refreshRequests++));

		const token = await tokenProvider.provideToken({ host: 'github.com', sessionId: 'session', reason: 'initial' });
